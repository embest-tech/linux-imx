...
static int bd71805_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	ret = regmap_read(mfd->regmap, BD71805_REG_INT_EN_12, &int_val);
	if (ret < 0)
		return ret;
...
static irqreturn_t bd71805_rtc_interrupt(int irq, void *rtc)
{
	...
	ret = regmap_read(mfd->regmap, BD71805_REG_INT_STAT_12, &rtc_reg);
	if (ret)
		return IRQ_NONE;
...
static int bd71805_rtc_probe(struct platform_device *pdev)
{
	...
	ret = regmap_read(bd71805->regmap, BD71805_REG_INT_STAT_12, &rtc_reg);
	if (ret < 0)
		return ret;

	ret = regmap_write(bd71805->regmap, BD71805_REG_INT_STAT_12, rtc_reg);
	if (ret < 0)
		return ret;
