...
//#include "board-mx6q_riot.h"
#include "board-mx6dl_riot.h"
#include <mach/imx_rfkill.h>

#define RIOT_CSI0_PWN	IMX_GPIO_NR(3, 20)
...
#define RIOT_USBH1_PWR_EN	IMX_GPIO_NR(1, 29)
...
#define RIOT_PCIE_PWR_EN	IMX_GPIO_NR(3, 19)
...
#define RIOT_DISP_PWR_EN	IMX_GPIO_NR(6, 14)
...
{
	if (powerdown)
		gpio_set_value(RIOT_CSI0_PWN, 1);
	else
		gpio_set_value(RIOT_CSI0_PWN, 0);
...
static void mx6q_csi0_io_init(void)
{
	...
	/* Camera power down */
	gpio_request(RIOT_CSI0_PWN, "cam-pwdn");
	gpio_direction_output(RIOT_CSI0_PWN, 1);
	msleep(5);
	gpio_set_value(RIOT_CSI0_PWN, 0);
	msleep(5);
...
static struct imxi2c_platform_data mx6q_riot_i2c_data = {
	.bitrate = 100000,
};
...
{
	gpio_set_value(RIOT_DISP_PWR_EN, 1);
	gpio_set_value(RIOT_DISP_RST_B, 1);
	udelay(10);
	gpio_set_value(RIOT_DISP_RST_B, 0);
	udelay(50);
	gpio_set_value(RIOT_DISP_RST_B, 1);
...
static struct gpio_keys_button riot_buttons[] = {
//	GPIO_BUTTON(RIOT_VOLUME_UP, KEY_VOLUMEUP, 1, "volume-up", 0, 1),
	GPIO_BUTTON(RIOT_VOLUME_DN, KEY_POWER, 1, "volume-down", 1, 1),
};

static struct gpio_keys_platform_data riot_button_data = {
	.buttons	= riot_buttons,
	.nbuttons	= ARRAY_SIZE(riot_buttons),
};

static struct gpio_keys_button new_riot_buttons[] = {
//	GPIO_BUTTON(RIOT_VOLUME_UP, KEY_VOLUMEUP, 1, "volume-up", 0, 1),
	GPIO_BUTTON(RIOT_VOLUME_DN, KEY_VOLUMEDOWN, 1, "volume-down", 0, 1),
	GPIO_BUTTON(RIOT_POWER_OFF, KEY_POWER, 1, "power-key", 1, 1),
};

static struct gpio_keys_platform_data new_riot_button_data = {
	.buttons	= new_riot_buttons,
	.nbuttons	= ARRAY_SIZE(new_riot_buttons),
};

static struct platform_device riot_button_device = {
	.name		= "gpio-keys",
	.id		= -1,
	.num_resources  = 0,
};
...
static void __init imx6q_add_device_buttons(void)
{
	 ...
	 *
	 * With the new riot change and SW mapping the
	 * SW1 as power key, below function related to power
	 * key are OK on new riot board(B4 or above).
	...
	if (mx6q_revision() >= IMX_CHIP_REVISION_1_2 ||
		...
		platform_device_add_data(&riot_button_device,
				&new_riot_button_data,
				sizeof(new_riot_button_data));
	else
		platform_device_add_data(&riot_button_device,
				&riot_button_data,
				sizeof(riot_button_data));

	platform_device_register(&riot_button_device);
}
#else
static void __init imx6q_add_device_buttons(void) {}
#endif
...
static struct platform_pwm_backlight_data mx6_riot_pwm_backlight_data = {
	.pwm_id = 3,
	.max_brightness = 248,
	.dft_brightness = 128,
	.pwm_period_ns = 50000,
				   ...
				   char **cmdline, struct meminfo *mi)
{
	...
	struct tag *t;
	int i = 0;
	struct ipuv3_fb_platform_data *pdata_fb = riot_fb_data;
...
static void __init mx6_riot_board_init(void)
{
	...
	imx6q_add_imx_i2c(0, &mx6q_riot_i2c_data);
	imx6q_add_imx_i2c(1, &mx6q_riot_i2c_data);
	imx6q_add_imx_i2c(2, &mx6q_riot_i2c_data);
	imx6q_add_imx_i2c(3, &mx6q_riot_i2c_data);
	...
	*/
	...
	imx6q_add_mxc_pwm(2);
	imx6q_add_mxc_pwm(3);
	imx6q_add_mxc_pwm_backlight(3, &mx6_riot_pwm_backlight_data);
...
static void __init mx6q_riot_reserve(void)
{
	 ...
	 */
	fb_array_size = ARRAY_SIZE(riot_fb_data);
	if (fb_array_size > 0 && riot_fb_data[0].res_base[0] &&
	    riot_fb_data[0].res_size[0]) {
		if (riot_fb_data[0].res_base[0] > SZ_2G)
			printk(KERN_INFO"UI Performance downgrade with FB phys address %x!\n",
			    riot_fb_data[0].res_base[0]);
		memblock_reserve(riot_fb_data[0].res_base[0],
				 riot_fb_data[0].res_size[0]);
		memblock_remove(riot_fb_data[0].res_base[0],
				riot_fb_data[0].res_size[0]);
		riot_fb_data[0].late_init = true;
		ipu_data[ldb_data.ipu_id].bypass_reset = true;
		fb0_reserved = 1;
	}
	for (i = fb0_reserved; i < fb_array_size; i++)
		if (riot_fb_data[i].res_size[0]) {
			/* Reserve for other background buffer. */
			phys = memblock_alloc_base(riot_fb_data[i].res_size[0],
						SZ_4K, SZ_2G);
			memblock_remove(phys, riot_fb_data[i].res_size[0]);
			riot_fb_data[i].res_base[0] = phys;
		}
